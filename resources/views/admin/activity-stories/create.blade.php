...
<div class="max-w-4xl mx-auto">
    <form action="{{ route('admin.activity-stories.store') }}" method="POST" class="space-y-6">
        @csrf
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 md:p-8">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <input type="text" name="title" value="{{ old('title') }}" placeholder="Contoh: Sumbangan Makanan Semasa Banjir" required
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" name="tag" value="{{ old('tag') }}" placeholder="Contoh: Bencana"
                                   class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 outline-none">
                        </div>
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-1">Tarikh (Teks)</label>
                            <input type="text" name="event_date" value="{{ old('event_date') }}" placeholder="Contoh: 15 Feb 2026"
                                   class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 outline-none">
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="number" name="sort_order" value="{{ old('sort_order', 0) }}" required
                               class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 outline-none">
                        <p class="text-[10px] text-gray-400 mt-1 uppercase tracking-wider italic">Gunakan nombor genap bagi item di kiri dan ganjil bagi kanan (Jika mahu kawal secara manual).</p>
                    </div>
                </div>
                    ...
                    <div x-data="imageUpload()" class="space-y-2">
                        ...
                        <div @click="$refs.fileInput.click()" 
                            ...
                            <div x-show="!imageUrl && !uploading" class="text-center">
                                ...
                                <p class="text-xs text-gray-400 font-medium font-mono uppercase tracking-widest">Pilih Gambar</p>
                            </div>
                            ...
                            {{-- Edit overlay --}}
                            <div x-show="imageUrl" class="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition flex items-center justify-center">
                                <span class="bg-white/20 backdrop-blur-md px-4 py-2 rounded-lg text-white text-xs font-bold uppercase tracking-wider border border-white/30">Tukar Gambar</span>
                            </div>
                        </div>
            ...
            <div class="mt-6">
                ...
                <textarea name="description" rows="4" required placeholder="Ceritakan sedikit tentang aktiviti ini..."
                          class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 outline-none @error('description') border-red-500 @enderror">{{ old('description') }}</textarea>
        ...
        <div class="flex items-center justify-end gap-3">
            ...
            <button type="submit" class="px-8 py-2 bg-blue-600 text-white font-bold rounded-lg hover:bg-blue-700 transition shadow-md shadow-blue-200">Simpan Cerita</button>
        </div>
    </form>
</div>
@endsection
