...
<div class="bg-white rounded-2xl shadow-xl shadow-gray-100/50 border border-gray-100 overflow-hidden transition-all duration-500">
    ...
    <div class="p-6 border-b border-gray-50 bg-gray-50/30 flex flex-col lg:flex-row items-center gap-6">
        <div class="flex-1 w-full">
            ...
            <p class="text-[10px] text-gray-400 font-medium">Urus dan lihat borang yang dihantar oleh ahli STU</p>
        </div>
        ...
        <table class="min-w-full text-left text-sm">
            <thead class="bg-gray-50/80 text-[10px] text-gray-400 uppercase tracking-[0.2em] font-black border-b border-gray-100">
                <tr>
                    ...
                    <th class="px-6 py-4">Jenis Borang</th>
            ...
            <tbody class="divide-y divide-gray-50">
                ...
                <tr class="hover:bg-blue-50/30 transition group/row animate-in fade-in duration-500 sub-row" 
                    data-search="{{ strtolower($sub->name . ' ' . $sub->email . ' ' . $sub->subject) }}">
                    <td class="px-6 py-4">
                        <div class="flex items-center gap-4">
                            <div class="w-10 h-10 bg-teal-50 text-teal-600 rounded-xl flex items-center justify-center shadow-sm border border-teal-100 transition group-hover/row:scale-110">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                                </svg>
                            </div>
                            <div>
                                <p class="font-bold text-gray-800 group-hover/row:text-blue-600 transition">{{ $sub->name }}</p>
                                <p class="text-[10px] text-gray-400 font-medium">{{ $sub->subject }}</p>
                            </div>
                        </div>
                    </td>
                    <td class="px-6 py-4">
                        <p class="text-xs font-semibold text-gray-700">{{ $sub->formType ? $sub->formType->name : 'N/A' }}</p>
                    ...
                    <td class="px-6 py-4">
                        ...
                        @endphp
                        <span class="inline-flex items-center px-2 py-0.5 text-[8px] font-black rounded-full {{ $statusClasses[$sub->status] ?? 'bg-gray-100 text-gray-400' }} uppercase tracking-widest border">
                            {{ $statusLabels[$sub->status] ?? $sub->status }}
                        </span>
                    </td>
                    <td class="px-6 py-4">
                        <div class="flex flex-col">
                            <span class="text-xs font-bold text-gray-700">{{ $sub->created_at->format('d M Y') }}</span>
                            <span class="text-[10px] text-gray-400 font-medium">{{ $sub->created_at->format('H:i A') }}</span>
                        </div>
                    </td>
                    <td class="px-6 py-4 text-right">
                        <div class="flex items-center justify-end gap-2 px-2">
                            ...
                            <button type="button" onclick="confirmDelete({{ $sub->id }}, '{{ addslashes($sub->name) }}')" 
                ...
                <tr>
                    <td colspan="5" class="px-6 py-24 text-center">
                        <div class="flex flex-col items-center">
                            ...
                            <p class="text-xs text-gray-400 mt-1">Belum ada sebarang borang yang dihantar oleh ahli.</p>
                        </div>
                    </td>
                </tr>
                @endforelse
