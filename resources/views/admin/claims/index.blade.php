...
<div class="bg-white rounded-2xl shadow-xl shadow-gray-100/50 border border-gray-100 overflow-hidden transition-all duration-500">
    ...
    <div class="p-6 border-b border-gray-50 bg-gray-50/30 flex flex-col lg:flex-row items-center gap-6">
        <div class="flex-1 w-full">
            ...
            <p class="text-[10px] text-gray-400 font-medium">Urus senarai pampasan dan bukti tuntutan ahli</p>
        </div>

        <form id="filter-form" method="GET" action="{{ route('admin.claims.index') }}" class="flex flex-col sm:flex-row flex-[2] gap-3 w-full">
            {{-- Dynamic Search --}}
            <div class="relative flex-1 group">
                <input type="text" name="search" id="search-input" value="{{ request('search') }}" 
                       placeholder="Cari nama ahli, sekolah atau jumlah..."
            ...
            <a href="{{ route('admin.claims.index') }}" id="reset-btn"
    ...
    <div id="table-container">
        @include('admin.claims.partials.table')
    </div>
</div>
@endsection
...
<script>
    ...
    function confirmDelete(id, title) {
        Swal.fire({
            title: 'Padam Rekod Tuntutan?',
            html: `Adakah anda pasti ingin memadam rekod pampasan untuk <strong>"${title}"</strong>?`,
            icon: 'warning',
