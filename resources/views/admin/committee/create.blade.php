...
@section('title', 'Tambah Pemimpin Baru')
@section('header', 'Tambah Pemimpin Baru')
...
<div class="max-w-4xl mx-auto">
    <div class="bg-white rounded-2xl shadow-xl shadow-gray-100/50 border border-gray-100 overflow-hidden transition-all duration-500">
        <form action="{{ route('admin.committee.store') }}" method="POST" enctype="multipart/form-data" class="p-8 space-y-8">
            @csrf
                ...
                <div class="flex-shrink-0 w-full md:w-1/3">
                    ...
                    <div class="relative group">
                        <div class="w-full aspect-square rounded-[2rem] bg-gray-50 border-4 border-dashed border-gray-100 flex flex-col items-center justify-center overflow-hidden group-hover:border-blue-400 transition-colors">
                            <img id="image-preview" src="{{ asset('images/lelaki-pending.png') }}" class="w-full h-full object-cover">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <select name="type" id="type" required
                                    class="w-full px-4 py-3 text-sm bg-gray-50 border-0 rounded-xl focus:bg-white focus:ring-4 focus:ring-blue-500/5 outline-none transition-all duration-300">
                                <option value="TOP" {{ old('type') === 'TOP' ? 'selected' : '' }}>Ahli Tertinggi</option>
                                <option value="EXCO" {{ old('type') === 'EXCO' ? 'selected' : '' }}>Exco Bahagian</option>
                            </select>
                        </div>
                        <div>
            ...
            <div class="flex justify-end gap-3 pt-8 border-t border-gray-100">
                ...
                <button type="submit" class="px-10 py-3 bg-blue-600 text-white text-xs font-black rounded-xl hover:bg-blue-700 transition shadow-lg shadow-blue-500/20 active:scale-95 uppercase tracking-widest">Simpan Pemimpin</button>
            </div>
        </form>
    </div>
</div>
