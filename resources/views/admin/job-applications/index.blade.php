...
<div class="bg-white rounded-2xl shadow-xl shadow-gray-100/50 border border-gray-100 overflow-hidden transition-all duration-500">
    ...
    <div class="p-6 border-b border-gray-50 bg-gray-50/30 flex flex-col lg:flex-row items-center gap-6">
        <div class="flex-1 w-full">
            ...
            <p class="text-[10px] text-gray-400 font-medium">Urus dan lihat permohonan kerja daripada pemohon</p>
        </div>
    ...
    <div class="overflow-x-auto relative">
        <table class="min-w-full text-left text-sm">
            ...
            <tbody class="divide-y divide-gray-50">
                ...
                <tr class="hover:bg-blue-50/30 transition group/row animate-in fade-in duration-500 app-row" 
                    data-search="{{ strtolower($app->name . ' ' . $app->email) }}">
                    <td class="px-6 py-4">
                        <div class="flex items-center gap-4">
                            <div class="w-10 h-10 bg-indigo-50 text-indigo-600 rounded-xl flex items-center justify-center shadow-sm border border-indigo-100 transition group-hover/row:scale-110">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                                </svg>
                            </div>
                            <div>
                                <p class="font-bold text-gray-800 group-hover/row:text-blue-600 transition">{{ $app->name }}</p>
                                <p class="text-[10px] text-gray-400 font-medium">{{ $app->email }}</p>
                            </div>
                        </div>
                    </td>
                    <td class="px-6 py-4">
                        @if($app->job)
                            <p class="text-xs font-semibold text-gray-700">{{ $app->job->title }}</p>
                        @else
                            <span class="text-[10px] font-bold text-gray-400 uppercase">Permohonan Umum</span>
                        @endif
                    </td>
                    <td class="px-6 py-4">
                        @php
                        ...
                        @endphp
                        <span class="inline-flex items-center px-2 py-0.5 text-[8px] font-black rounded-full {{ $statusClasses[$app->status] ?? 'bg-gray-100 text-gray-400' }} uppercase tracking-widest border">
                            {{ $statusLabels[$app->status] ?? $app->status }}
                        </span>
                    </td>
                    <td class="px-6 py-4">
                        <div class="flex flex-col">
                            <span class="text-xs font-bold text-gray-700">{{ $app->created_at->format('d M Y') }}</span>
                            <span class="text-[10px] text-gray-400 font-medium">{{ $app->created_at->format('H:i A') }}</span>
                        </div>
                    </td>
                    <td class="px-6 py-4 text-right">
                        <div class="flex items-center justify-end gap-2 px-2">
                            <a href="{{ route('admin.kerjaya.show', $app->id) }}" 
                            ...
                            <button type="button" onclick="confirmDelete({{ $app->id }}, '{{ addslashes($app->name) }}')" 
                ...
                <tr>
                    <td colspan="5" class="px-6 py-24 text-center">
                        <div class="flex flex-col items-center">
                            ...
                            <p class="font-bold text-gray-800">Tiada permohonan dijumpai</p>
                            <p class="text-xs text-gray-400 mt-1">Belum ada sebarang permohonan kerjaya yang diterima.</p>
                        </div>
                    </td>
                </tr>
                @endforelse
